...
HOW TO RUN (from repo root):
    ...
    python src/extract_speeds.py   # first, if data/processed/nyc_speeds.npy doesn't exist yet
    python src/gwg_simulation.py

INPUT:
    data/processed/nyc_speeds.npy  (produced by src/extract_speeds.py)

OUTPUT FILES (written to results/figures/):
    ...
    fig8_av_real_time_readiness.png
"""
...
if not os.path.exists(DATA_PATH):
    raise FileNotFoundError(
        f"Could not find '{DATA_PATH}'. Run 'python src/extract_speeds.py' "
        f"from the repo root first (see README.md) — it reads parquet files "
        f"from data/raw/ and writes this file."
    )
NYC_SPEEDS = np.load(DATA_PATH)
