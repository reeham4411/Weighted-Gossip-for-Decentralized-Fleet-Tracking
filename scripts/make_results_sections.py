...
def main():
    ...
    with open(OUT_SECTIONS, "w", encoding="utf-8") as fh:
    ...
    with open("tests/test_gwg.py", encoding="utf-8") as fh:
        test_count = sum(1 for line in fh
    ...
    for t, why in [
        ...
        M.append(f"- `{t}` — {why}")
    with open(OUT_NUMBERS, "w", encoding="utf-8") as fh:
        fh.write("\n".join(M) + "\n")
